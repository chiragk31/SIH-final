...
const PodcastPage = () => {
  ...
  return (
    <div className="min-h-screen relative bg-background text-foreground transition-colors duration-300">
      ...
      <div className="container px-4 pt-24 pb-12 max-w-4xl mx-auto relative z-10">
        <div className="mb-12 text-center">
          <div className="inline-flex items-center justify-center p-4 rounded-2xl bg-card/50 backdrop-blur-sm border border-border mb-6 shadow-2xl shadow-blue-500/10">
            <Radio className="h-8 w-8 text-blue-500" />
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-foreground tracking-tight">
            AI Podcast Studio
          </h1>
          <p className="text-muted-foreground text-lg max-w-xl mx-auto">Turn any text or document into an immersive audio experience.</p>
        </div>

        <Card className="mb-12 border-border bg-card/40 backdrop-blur-xl shadow-2xl relative overflow-hidden">
          ...
          <CardContent className="space-y-6 relative">
            <Tabs defaultValue="text" value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="grid w-full grid-cols-2 mb-6 bg-muted border border-border">
                <TabsTrigger value="text" className="data-[state=active]:bg-background data-[state=active]:text-foreground flex items-center gap-2">
                  <FileText className="h-4 w-4" /> Text Input
                </TabsTrigger>
                <TabsTrigger value="file" className="data-[state=active]:bg-background data-[state=active]:text-foreground flex items-center gap-2">
                  <Upload className="h-4 w-4" /> Upload PDF
                </TabsTrigger>
              </TabsList>

              <TabsContent value="text" className="space-y-4">
                <Label htmlFor="text" className="text-muted-foreground">Paste your content</Label>
                <Textarea
                  id="text"
                  value={text}
                  ...
                  rows={8}
                  className="resize-none bg-background/50 border-border focus:border-blue-500/50 focus:ring-blue-500/20 text-foreground placeholder:text-muted-foreground"
                />
                <p className="text-xs text-muted-foreground text-right">{text.length} characters</p>
              </TabsContent>

              <TabsContent value="file" className="space-y-4">
                <div className="border-2 border-dashed border-border rounded-lg p-8 text-center hover:bg-muted/50 hover:border-blue-500/30 transition-colors bg-background/30">
                  <Input
                    type="file"
                    accept=".pdf,.txt"
                  ...
                  <Label htmlFor="file-upload" className="cursor-pointer flex flex-col items-center gap-2">
                    ...
                    <span className="text-lg font-medium text-foreground">Click to upload PDF or Text file</span>
        ...
        {audioUrl && (
          <Card className="border-border overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-700 mb-12 bg-card/80">
            <CardHeader className="bg-background/80 border-b border-border/50">
              <CardTitle className="flex items-center justify-between text-foreground">
                <span className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                  Now Playing: <span className="text-sm font-normal text-muted-foreground truncate max-w-[200px]">{currentTitle}</span>
                </span>
                <Button variant="ghost" size="sm" onClick={downloadPodcast} className="text-muted-foreground hover:bg-muted/50 hover:text-foreground">
            ...
            <CardContent className="p-0 bg-background/40">
              <div className="relative w-full h-64">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-background via-background/80 to-transparent flex items-center justify-center gap-6">
                  <Button
                    size="lg"
                    className="rounded-full w-16 h-16 bg-primary text-primary-foreground hover:bg-primary/90 hover:scale-110 transition-all shadow-[0_0_20px_rgba(0,0,0,0.2)] dark:shadow-[0_0_20px_rgba(255,255,255,0.2)]"
        ...
        <div className="space-y-6">
          ...
          {isHistoryLoading ? (
            ...
            <div className="grid gap-4">
              {history.map((podcast) => (
                <Card key={podcast.id} className="cursor-pointer bg-card/40 border-border hover:bg-card/60 hover:border-blue-500/30 transition-all group" onClick={() => playHistory(podcast.audio_url, podcast.title)}>
                  <CardContent className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-semibold text-foreground group-hover:text-blue-500 transition-colors">{podcast.title}</h3>
            ...
            <div className="text-center py-12 border-2 border-dashed border-border rounded-xl bg-card/20">
