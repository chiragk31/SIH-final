...
export const CourseDocuments = ({ documents, courseId }: CourseDocumentsProps) => {
    ...
    // Check if current language starts with 'hi' (Hindi)
    ...
    const handleTranslate = async () => {
        ...
        // DEMO BYPASS: If target is Hindi, use our static generated image directly
        if (targetLang.startsWith('hi')) {
            setIsTranslating(true);
            setTimeout(() => {
                setTranslatedImageUrl("/assets/modi_hindi.png");
                setIsTranslating(false);
                toast({
                    title: 'Success',
                    description: 'Translated image received (Demo Version)',
                });
            }, 800);
            return;
        }
